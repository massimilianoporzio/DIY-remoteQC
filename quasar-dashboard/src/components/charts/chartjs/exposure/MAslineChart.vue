...
<div>
    <q-card>
      <q-card-section>
        <Line chart-id="mAs" :chart-data="chartData" :chartOptions="chartOptions" height="300"></Line>
      </q-card-section>
...
onMounted(()=>{
  ...
  const mAsData =  props['apiData'].map(entry => Number(entry.mAs))
  chartData.labels = xLabels
  chartData.datasets[0].data = mAsData
  ...
  const baseline = Number(sortedBaselines[0].mAs)
  ...
  const yMax  =Math.max(...mAsData)
  const yMin  =Math.min(...mAsData)
  ...
  chartOptions.scales.y.suggestedMin = suggestedMin - 0.5*baseline
  chartOptions.scales.y.suggestedMax = suggestedMax + 0.5*baseline
