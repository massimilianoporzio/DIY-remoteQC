...
<div>
    <q-card>
      <q-card-section>
        <Line chart-id="snr" :chart-data="chartData" :chartOptions="chartOptions" height="200"></Line>
      </q-card-section>
...
onMounted(()=>{
  ...
  const SNRData =  props['apiData'].map(entry => Number(entry.SNR))
  chartData.labels = xLabels
  chartData.datasets[0].data = SNRData
  ...
  const baseline = Number(sortedBaselines[0].SNR)
  ...
  const yMax  =Math.max(...SNRData)
  const yMin  =Math.min(...SNRData)
