...
<div>
    <q-card>
      <q-card-section>
        <Line chart-id="mtf20" :chart-data="chartData" :chartOptions="chartOptions" height="200"></Line>
      </q-card-section>
...
onMounted(()=>{
  ...
  console.log("MTFdata: ",MTF20data)
  chartData.labels = xLabels
  chartData.datasets[0].data = MTF20data
  ...
  const baselineMTF20 = Number(sortedBaselines[0].MTF20)

  chartOptions.plugins.annotation.annotations.line1.yMin = baselineMTF20
  chartOptions.plugins.annotation.annotations.line1.yMax = baselineMTF20
  const yMax  =Math.max(...MTF20data)
  const yMin  =Math.min(...MTF20data)
  const upper = baselineMTF20*1.2
  const lower = baselineMTF20*0.8
  ...
  chartOptions.scales.y.suggestedMin = suggestedMin - 0.2
  chartOptions.scales.y.suggestedMax = suggestedMax + 0.2
