...
onMounted(()=>{
  ...
  const baseline = baselines.length ?  Number(sortedBaselines[0].EI) : 0
  ...
  const suggestedMax = yMax > upper ? yMax : upper + 50

  const suggestedMin = yMin < lower ? yMin : lower -50
