...
onMounted(()=>{
  ...
  const data = props['apiData'].map(entry => [new Date(entry.date).setHours(1,0,0,0,),
                                entry.mAs? Number(entry.mAs):null])
  ...
  const baseline = baselines.length ?  Number(sortedBaselines[0].mAs) : 0
  ...
  const suggestedMax = yMax > upper ? yMax : upper + 5

  const suggestedMin = yMin < lower ? yMin : lower -0.5
